...
function initNavigation() {
  ...
  /* ── Sticky header: always solid on mobile, transparent→solid on desktop ── */
  if (header) {
    const handleScroll = () => {
  ...
  if (hamburger && mobileMenu) {
    ...
    /* ── Close on tap outside menu or hamburger ── */
  ...
  highlightActiveNavLink(allNavLinks);
...
/**
 * Close the mobile navigation menu and reset hamburger state.
 */
function closeNav() {
  const mobileMenu = document.getElementById('mobile-menu') || document.querySelector('.mobile-menu');
  const hamburger  = document.getElementById('hamburger')   || document.querySelector('.hamburger');
