// ============================================================
// BHARAT WEB AZADARI â€” Service Worker (sw.js)
// ============================================================
// Strategy:
//   â€¢ Static assets (CSS, JS, images) â†’ Cache-First
//   â€¢ Navigation (HTML pages)          â†’ Network-First, fallback cache
//   â€¢ Google Apps Script API           â†’ Network-Only (never cache)
//   â€¢ YouTube RSS / allorigins         â†’ Network-First, 30 min cache
// ============================================================
...
// ============================================================
// INSTALL â€” Pre-cache all static assets
// ============================================================
  ...
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => {
        ...
        // Use addAll â€” if any asset fails, the whole install fails gracefully
...
// ============================================================
// FETCH â€” Route requests to appropriate strategy
// ============================================================
self.addEventListener('fetch', event => {
  const { request } = event;
  const url = new URL(request.url);

  // â”€â”€ 1. Never cache non-GET requests (POST, etc.) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  if (request.method !== 'GET') {
    return; // Let it fall through to the network
  }

  // â”€â”€ 2. Google Apps Script API â†’ Network-Only â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ 3. YouTube RSS / allorigins proxy â†’ Network-First, 30 min cache â”€â”€
  if (
  ...
  // â”€â”€ 4. Navigation requests (HTML pages) â†’ Network-First â”€â”€â”€
  if (request.mode === 'navigate') {
    event.respondWith(navigationStrategy(request));
    return;
  }

  // â”€â”€ 5. Google Fonts â†’ Cache-First (external, long-lived) â”€â”€
  if (
  ...
  // â”€â”€ 6. Static assets (CSS, JS, images) â†’ Cache-First â”€â”€â”€â”€â”€â”€
  const isStaticAsset = (
  ...
  // â”€â”€ 7. Anything else â†’ Network with cache fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function navigationStrategy(request) {
  ...
  try {
    ...
    console.warn('[SW] Navigation: Network failed for', request.url, 'â€” falling back to cache.');
    ...
    // Absolute fallback â€” plain text
    return new Response(
      `<!DOCTYPE html>
      <html lang="en">
      <head><meta charset="UTF-8"><title>Offline â€” BWA</title></head>
      <body style="background:#0B0B0B;color:#D4AF37;font-family:sans-serif;text-align:center;padding:60px 20px;">
        <h1>You're Offline</h1>
        <p>Please check your internet connection and try again.</p>
        <a href="/" style="color:#D4AF37;">â† Go Home</a>
      </body>
      </html>`,
...
/**
 * YouTube Cache Strategy â€” Network-First with 30-minute expiry
...
async function youtubeCacheStrategy(request) {
  ...
  // Cache is stale or missing â€” fetch from network
  try {
    const networkResponse = await fetch(request);
    if (networkResponse && networkResponse.status === 200) {
